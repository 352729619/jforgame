...
public enum OrmProcessor {
        ...
        for (Field field:fields) {
            ...
            try{
                ...
                if (!StringUtils.isEmpty(column.name())) {
                    ...
                    //
                    bridge.addColumnAnnotation(column.name(),column);
                    bridge.addGetterMethod(column.name(), m);
                }
                bridge.addProperty(fieldName);
                bridge.addColumnAnnotation(fieldName,column);
            }catch(Exception e) {
    ...
    public Map<Class<?>, OrmBridge> getClassOrmMapperr(){
        return Collections.unmodifiableMap(classOrmMapperr);
    }
}
